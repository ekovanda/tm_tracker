...
- `static/` contains the static web frontend assets (`index.html`, `styles.css`, `app.js`) establishing a dark-themed UI layout with password unlock gate, 4x4 Bingo board grid, player timer cards, live PB record history feed, and client-side timer countdown calculations that derive elapsed time from server timestamps to decouple the 1-second UI tick from network traffic.
