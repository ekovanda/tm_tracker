...
2. Before a session exists, the page loads official campaigns and renders the pre-session settings panel for campaign, player timer duration, grace period, and maximum game length.
...
- `BingoState.settings` stores immutable per-session timing settings, including the overall session duration, opening grace period, and player timer duration.
...
- Session settings are passed into `start_bingo` and retained in `BingoState`; the pre-session configuration UI is separate from the domain enforcement.
...
- The pre-session settings panel is only rendered when no `BingoSession` exists. Active sessions expose stop/reset controls and retain the selected timing values in their immutable state.
