    ...
    def get_rank_counts(self, all_pbs: dict) -> None:
        ...
        for pb_dict in all_pbs:
            ...
            if len(players) > 0 and players[0]["player"].account_id == self.account_id:
                ...
                len(players) > 1 and players[1]["player"].account_id == self.account_id
            ):
                self.medal_count["silver"] += 1
            elif (
                len(players) > 2 and players[2]["player"].account_id == self.account_id
            ):
...
PLAYERS: list[Player] = [
    ...
    Player(
        name="LryKo", account_id="f00ab19a-14e5-4e6e-9a45-78f1af8487e1", alias="Lry"
    ),
    Player(
        name="Timo._.o", account_id="b9b79f89-19e7-4f26-be01-03556b8890b9", alias="Timo"
    ),
]
